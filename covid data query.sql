...
---Average cases and deaths in 2020 - India

select month(date) as month, round(avg(total_cases),1) as Average_cases,avg(cast(total_deaths as int)) as Average_deaths
from new_project..covid_death
...
---Average deaths in a year in India by covid

select year(date) as Year, round(avg(total_cases),1) as Total_Average_cases,avg(cast(total_deaths as bigint)) as Total_Average_deaths
from new_project..covid_death
---where location = 'India' and 
---where date like '%2021%'
group by year(date)
...
select month(date) as month, sum(new_cases) as Total_new_cases,sum(cast(new_deaths as int)) as Total_new_deaths,round(sum(cast(new_deaths as int)) / sum(new_cases) *100, 3) as Death_percentage
from new_project..covid_death
...
select sub.location,
	   ...
	   sum(cast(sub.people_fully_vaccinated as bigint)) as People_vaccinated
from (
...
drop view if exists covid_data_percent

create VIEW 
covid_data_percent as
select death.continent,death.location,death.date,death.population,vaccine.new_vaccinations
,sum(convert(bigint,vaccine.new_vaccinations)) OVER(partition by death.location order by death.location,death.date) as cumulative_sum_of_vaccines
from new_project..covid_death death
join new_project..covid_vaccine vaccine
